      ...
      <el-container>
        <el-aside width="15%">
          <el-menu :router="true" ellipsis="false" active-text-color="#ffd04b" background-color="#fff">
            <el-menu-item @click="selectMenuItem('information')">
              <span>上传信息</span>
            </el-menu-item>
            <el-menu-item @click="selectMenuItem('application')">
              <span>信息管理员页面</span>
            </el-menu-item>
            <el-menu-item @click="selectMenuItem('check')">
              <span>查看上传信息状态</span>
            </el-menu-item>
          </el-menu>
        </el-aside>
        <el-main>
          <div v-if="selectedMenuItem === 'information'">
            <!-- 渲染上传信息组件 -->
            <el-alert title="请在此处提交信息，非常感谢您的帮助~" type="success" description="您可以上传图书馆内缺少的信息，成功通过审核后您就可以在网站中看到啦！" />
            <el-row>
              <el-card class="box-card" style="width: 45%; ">
                ...
                <label>类别:</label>
                <br>
                <el-input :required="true" v-model="category" placeholder="Please input category"
                  style="width: 60%;" /><label style="color: rgb(227, 72, 72);">*必填</label>
                <br>
                <label>品牌：</label>
                <br>
                ...
                <label>颜色：</label>
                <br>
                <el-input :required="true" v-model="colorway" placeholder="Please input colorway"
                  style="width: 60%;" /><label style="color: rgb(227, 72, 72);">*必填</label>
                <br>
                <label>特征:</label>
                <br>
          ...
          <div v-if="selectedMenuItem === 'application' && Adminstration == '1'">
            <!-- 渲染信息管理员页面组件 -->
            <el-text class="mx-1" type="primary">信息管理员在此处进行审核哦❥(^_-)</el-text>
                    ...
                    <div style="height: 15%;">
                      ...
                      <div style="height: 0%;">
                        ...
                        <div style="text-align: center;">
                          <router-link :to="'/audit/' + data[0] + '/' + user"> <el-button text
          ...
          <div v-if="selectedMenuItem === 'application' && Adminstration == '0'">

            <el-alert title="很抱歉您还不是信息管理员，但可以申请成为信息管理员哦！" type="success"
              description="信息管理员可以获得更多权限，可以审核裙子的信息哦~如果您想获得更多权限可以给网站负责人发邮件哦！" />
            <el-text class="mx-1" type="success">邮箱号：26453324400@qq.com</el-text>
            <ApplicationComponent></ApplicationComponent>
          </div>



          <div v-if="selectedMenuItem === 'check'">
            <!-- 渲染查看上传信息状态组件 -->
            <div v-if="false_newdata.length >= 1">
              <el-alert title="您提交的以下信息正在审核中" type="warning" />
              <el-progress :percentage="100" status="warning" :indeterminate="true" :duration="1" />
              <el-row>
                <el-col v-for="data in false_newdata " :key="data[1]" :span="6" :offset="2"
                  style=" margin-bottom: 5%; margin-left: 5%;">
                  <el-card shadow="always" style="height: 100%; width: 110%;">
                    <div style="padding: 0px;font-size: 100%; ">{{ data[1] }}</div>
                    <el-image style="height: 100%;width: 100%;" :src="data[7]" class="image" />
            ...
            <div v-else>
              <el-alert title="当前未存在您提交的正在审核中的信息~" type="info" />
            </div>

            <div v-if="true_newdata.length >= 1">
              <el-alert title="您提交的以下信息已经通过审核啦！" type="success" />
              <el-progress :percentage="100" status="success" :indeterminate="true" :duration="1" />
              <el-row>
                <el-col v-for="data in true_newdata " :key="data[1]" :span="6" :offset="2"
                  style=" margin-bottom: 5%; margin-left: 5%;">
                  <el-card shadow="always" style="height: 100%; width: 110%;">
                    <div style="padding: 0px;font-size: 100%; ">{{ data[1] }}</div>
                    <el-image style="height: 100%;width: 100%;" :src="data[7]" class="image" />
            ...
            <div v-else>
              <el-alert title="当前未存在您提交的已审核成功的信息~" type="info" />
            </div>
          </div>
        </el-main>
      </el-container>
...
import type { FormProps, menuEmits, UploadFile, UploadFiles, UploadProps, UploadRawFile } from 'element-plus'
...
const Adminstration = ref('')
const router = useRouter()
const user = ref('')

const fasle_data = ref([])
const true_data = ref([])
...
onBeforeMount(() => {
  ...
  status_true()
  search_audit()
  console.log(audit_newdata.value)
...
const selectedMenuItem = ref('information');
...
async function auditor_login() {
  let res = await axios.post("http://localhost:8888/auditor_login",
    {
      user: user.value,
    }
  )
  if (res.status == 200) {
    Adminstration.value = res.data
  }
}
...
async function status_false() {
  let res = await axios.post("http://localhost:8888/status_false",
    {
      user: user.value,
    }
  )
  if (res.status == 200) {
    fasle_data.value = res.data
  }
}

async function status_true() {
  let res = await axios.post("http://localhost:8888/status_true",
    {
      user: user.value,
    }
  )
  if (res.status == 200) {
    true_data.value = res.data
  }
}
...
const false_newdata = computed(() => {
  return fasle_data.value.slice((page.value - 1) * 10, page.value * 10 - 1)

})
const true_newdata = computed(() => {
  return true_data.value.slice((page.value - 1) * 10, page.value * 10 - 1)
